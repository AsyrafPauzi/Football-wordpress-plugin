...
class FLMS_Transfer_System {
    ...
    public function handle_actions() {
        ...
        if ( $_POST['flms_transfer_act'] === 'request' ) {
            ...
            if($current_team_id) {
                $mgr_id = get_post_field('post_author', $current_team_id);
                $mgr = get_userdata($mgr_id);
        ...
        if ( $_POST['flms_transfer_act'] === 'approve' ) {
            ...
            // Verify Owner
            $from_team = get_post_meta($req_id, '_from_team', true);
            $team_post = get_post($from_team);
            if($team_post->post_author != get_current_user_id()) wp_die('Unauthorized');
    ...
    private function notify_requesting_manager_for_payment($req_id) {
        $to_team = get_post_meta($req_id, '_to_team', true);
        $mgr_id = get_post_field('post_author', $to_team);
        $mgr = get_userdata($mgr_id);
    ...
    public function execute_paid_actions( $order_id ) {
        ...
        foreach ( $order->get_items() as $item ) {
            ...
            if ( $pid && $tid ) {
                ...
                if(class_exists('FLMS_Player_Stats')) (new FLMS_Player_Stats())->recalculate_single_player($pid);
